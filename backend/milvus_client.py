...
class MilvusManager:
    ...
    def init_collection(self, dense_dim: int = 2560):
        ...
        if not self._get_client().has_collection(self.collection_name):
            schema = self._get_client().create_schema(auto_id=True, enable_dynamic_field=True)
    ...
    def drop_collection(self):
        """删除集合（用于重建 schema）"""
        if self._get_client().has_collection(self.collection_name):
            self._get_client().drop_collection(self.collection_name)
